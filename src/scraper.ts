...
export default class Scraper {
  ...
  async bypassAndGetVideoUrl(iframeUrl: string): Promise<string> {
    ...
    try {
      ...
      if (nestedIframeMatch) {
          ...
          if (realVideoHost.startsWith('//')) realVideoHost = `https:${realVideoHost}`;
  ...
  private async fetchDirectVideoLink(videoHostUrl: string, parentReferer: string): Promise<string> {
    try {
        ...
        if (!videoId) throw new Error('Video ID tidak ditemukan di URL Layer 2.');
        ...
        const jsContent = jsResponse.body as string;
        ...
        const resData: any = apiResponse.body;
        if (resData.message) {
        ...
        if (Array.isArray(resData)) {
            const firstSource = resData.find((src: any) => src.file);
            if (firstSource) directUrl = firstSource.file;
        } else if (resData && typeof resData === 'object') {
            if (resData.file) directUrl = resData.file;
            else if (Array.isArray(resData.sources)) {
                const firstSource = resData.sources.find((src: any) => src.file);
                if (firstSource) directUrl = firstSource.file;
            }
        }
        ...
        if (!directUrl) {
            ...
            if (m3u8Match) directUrl = m3u8Match[0].replace(/\\/g, '');
            
            const mp4Match = rawJsonString.match(/https?:\/\/[^"'\s<>,\\]+\.mp4[^"'\s<>,\\]*/i);
            if (mp4Match && !directUrl) directUrl = mp4Match[0].replace(/\\/g, '');
        }
