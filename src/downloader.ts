...
export default class Downloader {
  ...
  async downloadM3U8(m3u8Url: string, customHeaders: Record<string, string> = {}): Promise<Readable> {
    ...
    let manifestText = manifestRes.body as string;
    ...
    if (manifestText.includes('#EXT-X-STREAM-INF')) {
      ...
      if (targetMediaUrl) {
        ...
        manifestText = manifestRes.body as string;
    ...
    const downloadChunk = async (segmentUrl: string, index: number): Promise<string> => {
